...
{ 
    ...
    public class Wip_InStockRecordController : ControllerBase 
    { 
        private readonly IWip_InStockRecordServices _Wip_InStockRecordServices; 
        private readonly IUser _user; 
 
        public Wip_InStockRecordController(IWip_InStockRecordServices Wip_InStockRecordServices, IUser user) 
        { 
        ...
        [HttpPost] 
        public async Task<MessageModel<PageModel<Wip_InStockRecord>>> Get([FromBody] Wip_InStockRecordParam param) 
        { 
            ...
            var whereConditions = WhereConditionsExtensions.GetWhereConditions<Wip_InStockRecord, Wip_InStockRecordParam>(param); 
            if (!whereConditions.IsSuccess) { 
                return new MessageModel<PageModel<Wip_InStockRecord>>() 
                { 
        ...
        /// <returns>获取结果</returns> 
        [HttpGet] 
        public async Task<MessageModel<Wip_InStockRecord>> Get(int id = 0) 
        { 
            return new MessageModel<Wip_InStockRecord>() 
            { 
                msg = "获取成功", 
                success = true, 
        ...
        /// <returns></returns> 
        [HttpPost] 
        public async Task<MessageModel<string>> Post([FromBody] Wip_InStockRecord request) 
        { 
        ...
        [HttpPut] 
        public async Task<MessageModel<string>> Put([FromBody] Wip_InStockRecord request) 
        { 
            var data = new MessageModel<string>(); 
            if (request.Id > 0) 
            { 
