...
import { getAgvListPage, removeAgv, editAgv, addAgv } from '../../api/api';
...
export default {
  ...
  data () {
    return {
      ...
      //导出组件相关 
      export_exportFileName: 'Agv车辆信息',//要导出的文件名 
      export_currentPageData: [],//当前页面的列表数据 
      export_columnHeader: { 'AgvName': '车辆名称', 'AgvNameDesc': '车辆描述', 'Remark': '备注', 'EnergyLevel': '电量', 'CurrentPosition': '当前位置', 'XCoordinate': 'X坐标', 'YCoordinate': 'Y坐标', 'State': '状态', 'StateName': '状态', 'TransportOrder': '当前执行的任务', 'CreateBy': '创建者', 'CreateTime': '创建时间', 'ModifyBy': '修改者', 'ModifyTime': '修改时间' },//当前页面列表的表头汉字和属性数组，导出用  
      ...
      c_searchFormInputPlaceholder: '请输入车辆名称/车辆描述/备注/当前位置/X坐标/Y坐标/状态/状态/当前执行的任务/操作说明/创建者/修改者',//要给子搜索组件传递的值 
      c_searchFormInputAttrs: ['AgvName', 'AgvNameDesc', 'Remark', 'CurrentPosition', 'XCoordinate', 'YCoordinate', 'State', 'StateName', 'TransportOrder', 'OperationRemark', 'CreateBy', 'ModifyBy'],//要给子搜索组件传递的属性名 
  ...
  methods: {
    ...
    getAgvs (formValue) {
      ...
      getAgvListPage(para).then((res) => {
        if (res.data.success) {
          this.total = res.data.response.dataCount;
          this.Agvs = res.data.response.data;
        } else {
          this.$message({
    ...
    //导出 
    handleExport () {
      this.export_currentPageData = this.Agvs
