...
import { getForNoFinished, removeAgvTask, editAgvTask, addAgvTask, getEnumberList } from '../../api/api';
import { getButtonList } from "../../promissionRouter";
import Toolbar from "../../components/ToolbarButton";
import SearchForm from "../../components/SearchForm";
import ToolbarExport from "../../components/ToolbarExport";
import ShowAgvTaskPhaseList from "../LES/LESComponents/ShowAgvTaskPhaseList.vue";
import ShowTaskPhaseList from "../LES/LESComponents/ShowTaskPhaseList";
...
export default {
  ...
  data () {
    return {
      ...
      export_columnHeader: { 'TaskNo': 'AGV任务号', 'LesTaskNo': 'LES任务号', 'TaskType': '任务类型', 'TaskTypeName': '任务类型名称', 'CallOrderNo': '叫料单号', 'SourcePlace': '起始点', 'SourcePlaceName': '起始点名称', 'ToPlace': '目标点', 'ToPlaceName': '目标点名称', 'MaterialId': '物料表ID', 'MaterialCode': '物料号', 'MaterialName': '物料名称', 'CargoNum': '货物数量', 'SalverCode': '托盘号', 'TaskStatus': '任务状态', 'TaskCreateTime': '任务创建时间', 'TaskIssueTime': '下发时间', 'TaskFinishTime': '完成时间', 'CreateBy': '创建者', 'CreateTime': '创建时间', 'ModifyBy': '修改者', 'ModifyTime': '修改时间', 'IsDeleted': '是否逻辑删除' },//当前页面列表的表头汉字和属性数组，导出用  
      //搜索框相关 
      c_commonSearchOptionSet: "模糊",//通用查询的默认配置,"模糊"或"精准" 
      c_searchValControlStyle: {//设置通用搜索框的长度等样式  
        width: '300px',
      },
      c_searchFormInputPlaceholder: '请输入车辆/AGV任务号/LES任务号/叫料单号/起始点编号/目标点编号/起始点名称/目标点名称/工件号/工件名称/托盘号/操作说明/创建者/修改者',//要给子搜索组件传递的值 
      c_searchFormInputAttrs: ['AgvNameDesc', 'TaskNo', 'LesTaskNo', 'CallOrderNo', 'SourceAgvStation', 'ToPlaceAgvStation', 'SourcePlaceName', 'ToPlaceName', 'MaterialCode', 'MaterialName', 'SalverCode', 'OperationRemark', 'CreateBy', 'ModifyBy'],//要给子搜索组件传递的属性名 
  ...
  methods: {
    ...
    getAgvTasks (formValue) {
      ...
      getForNoFinished(para).then((res) => {
        if (res.data.success) {
