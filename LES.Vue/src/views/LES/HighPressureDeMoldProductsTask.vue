...
import {
  getMaterialsCodeItemsByProcessAndStation,
  coerceFinishAgvTask, cancelAgvTask, getCodeItemsBySetCodeForGX, getCodeItemsByCode, getEnumberList,
  getAllStationList, getCodeItemsBySetCode,
  getHighPressureProductsTaskListPage, editStockInfo, addHighPressureProductsTask
} from '../../api/api';
  ...
  data () {
    return {
      ...
      rules: {
        ...
        //     var msg = validateNewBurdenWorkNo(this.default_Material.Id, '', newBurdenWorkNo
        //       , this.default_Material.code, this.default_Material.name)
        //     if (msg != '') {
      ...
      default_Material: {},//当前页面默认的物料

      currentLesTaskType: 13, // 高压脱模运送任务
      StoreAreaCode: 'KCQY_GYTMFHQ', // 高压脱模发货区
      InStoreSceneEnum: 0, // 这个字段可以不用
      my_dialogTitle: '高压脱模发货',
      default_AllowProcedure: 'GX_WGX',//默认的工序-- 无工序
    }
  },
  methods: {
    ...
    changeStation: function (selVal) {
      ...
      let para = {
        processSetCode: this.default_AllowProcedure, // 工序
        stationId: stationId // 站点
      }
    ...
    _getEmptyStationList: function () {
      ...
      let para = {
        ...
        InStoreSceneEnum: this.InStoreSceneEnum
      }
    ...
    getPostParam (flag) {
      ...
      //para.LesTaskTypes.push(this.currentLesTaskType2)
      return para
    },
    _getStockInfos () {
      this.page = 1
    ...
    handleAdd () {
      ...
      this.isAdd = true
      this.dialogTitle = this.my_dialogTitle
      this.addFormVisible = true;
      this.setInitAddForm()
    ...
    getMaterCodeItem (_code) {
      let para = {
        code: _code
      }
      getCodeItemsByCode(para).then((res) => {
        if (res.data.success) {
          this.default_Material = res.data.response
        } else {
          this.$message({
            message: res.data.msg,
            type: 'error'
