    ...
    <el-dialog :title="dialogTitle"
      ...
      <el-form :model="addForm"
        ...
        <el-form-item label="起始站点"
                      prop="StationId"
                      :rules="[{ required: true, message: '起始站点不能为空'}]">
        ...
        <el-form-item label="目标站点"
                      prop="ToStationId"
                      :rules="[{ required: true, message: '目标站点不能为空'}]">
          <el-select v-model="addForm.ToStationId"
                     clearable
                     filterable
                     style="width:100%"
                     placeholder="请选择">
...
import {
  ...
  getAllStationsByUserProcedure, getCodeItemsBySetCode,
...
export default {
  ...
  data () {
    return {
      ...
      currentLesTaskType: 7, // 7 是自由搬运任务
    }
  },
  methods: {
    //显示 AGV任务阶段列表
    ...
    _getEmptyStationList: function (processName) {
      ...
      getAllStationsByUserProcedure(para).then(res => {
        if (res.data.success) {
    ...
    SaveData (formName) {
      ...
      this.$refs.addForm.validate((valid) => {
        if (valid) {
          this.$confirm('请确定好起始站点有物料，目标站点没有物料,您确认提交吗？', '提示', {}).then(() => {
            this.addLoading = true;
  ...
  mounted () {
    ...
    //获取默认物料
    this.getMaterCodeItem('GX_GYRX_GYRXXQ') //高压绕线线圈
    this.setInitAddForm()
