<template>
  <section>
    ...
    <el-table :data="StockInfos"
      ...
      <el-table-column prop="LowPressureMoldTaskFlag"
                       label="类型"
                       width="150"
                       align="center">
      </el-table-column>
    ...
    <el-dialog :title="dialogTitle"
      ...
      <el-form :model="addForm"
        ...
        <el-form-item label="类型"
                      prop="LowPressureMoldTaskFlag"
                      :rules="[{ required: true, message: '类型不能为空'}]">
          <el-select v-model="addForm.LowPressureMoldTaskFlag"
                     clearable
                     style="width:100%"
                     placeholder="请选择">
            <el-option v-for="item in LowPressureMoldTaskFlagItems"
                       @click.native="changeLowPressureMoldTaskFlag"
                       :key="item.value"
                       :label="item.value"
                       :value="item.value">
            </el-option>
        ...
        <el-form-item label="生产任务号"
                      ...
                      prop="BurdenWorkNo"
                      :rules="[{ required: true, message: '生产任务号不能为空'}]">
...
export default {
  ...
  data () {
    return {
      ...
      currentLesTaskType: 5 // 5 是低压内膜运送任务
    }
  },
  methods: {
    //显示 AGV任务阶段列表
    ...
    _getEmptyStationList: function (processName) {
      let para = {
        ...
        StoreAreaCode: 'KCQY_DYRXNMQ', // 低压绕线机内膜区
        InStoreSceneEnum: 5 // 5 是 低压内模送货
      }
    ...
    // 选择 类型
    changeLowPressureMoldTaskFlag () {
      if (this.addForm.LowPressureMoldTaskFlag == '叫料') {
