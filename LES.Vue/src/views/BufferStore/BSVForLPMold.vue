    ...
    <el-dialog title="站点操作"
      ...
      <el-form :model="mainForm"
        ...
        <el-form-item label="生产任务号"
                      prop="BurdenWorkNo">
          <el-input v-model="mainForm.BurdenWorkNo"></el-input>
        </el-form-item>
      ...
      <div slot="footer"
        ...
        <el-button type="danger"
                   class="footSty"
                   icon="fa fa-cubes"
                   @click="doClick(6)">增加低压内膜库存</el-button>
...
import { addStoreByStation, updateStationIsLock, updateStationIsDeleted, getStockByArea, deleteStockByStationId, getCodeItemsBySetCodeForGX, getStockInfoCollectListPage } from '../../api/api';
...
export default {
  ...
  methods: {
    ...
    handleAddStoreByStation () {
      if (this.mainForm.BurdenWorkNo == null || this.mainForm.BurdenWorkNo == '') {
        this.$message({
          message: '请输入生产任务号',
          type: 'warning'
        });
      ...
      this.$confirm("您确定要将站点[" + this.mainForm.StationName + "] 增加低压内膜物料 吗?", '提示', {
        type: 'warning'
      }).then(() => {
        this.listLoading = true;
        let para = {
          CodeItemsByCode: 'GGWL_DYNM',//低压内膜的物料
          Remark: this.mainForm.OperationRemark,
          StationId: this.mainForm.StationId,
          BurdenWorkNo: this.mainForm.BurdenWorkNo //增加 生产任务号
        };
    ...
    getStockInfos (formValue) {
      this.listLoading = true;
      var param = {
        storeAreaCode: 'KCQY_DYNMHCQ' //低压内膜缓存区
      }
      getStockByArea(param).then((res) => {
        if (res.data.success) {
