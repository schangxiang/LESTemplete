  ...
  <section>
    ...
    <el-col :span="24"
      ...
      <el-form :inline="true"
        ...
        <SearchForm :commonSearchOptionSet="c_commonSearchOptionSet"
                    ...
                    ref="ChildSearchForm"
                    @onSearch="_getInOutStockRecords" />
      </el-form>
    </el-col>

    <!--列表-->
    <el-table :data="InOutStockRecords"
      ...
      <el-table-column prop="InOutStockTypeName"
                       width="100"
                       label="出入库类型"
                       align="center">
      </el-table-column>
    ...
    <el-dialog :title="dialogTitle"
      ...
      <el-form :model="addForm"
        ...
        <el-form-item label="出入库类型"
                      prop="InOutStockType"
                      :rules="[{ required: true, message: '出入库类型不能为空'}]">
          <el-input-number v-model="addForm.InOutStockType"></el-input-number>
        </el-form-item>
        <el-form-item label="出入库类型"
                      prop="InOutStockTypeName"
                      :rules="[{ required: true, message: '出入库类型不能为空'}]">
          <el-input v-model="addForm.InOutStockTypeName"></el-input>
        </el-form-item>
        ...
        <el-form-item label="站点ID"
          ...
          <el-input-number v-model="addForm.StationId"></el-input-number>
        </el-form-item>
    ...
    <el-dialog title="查看"
      ...
      <el-form :model="viewDetailForm"
        ...
        <el-form-item label="出入库类型"
                      prop="InOutStockType">
          <el-input-number v-model="viewDetailForm.InOutStockType"></el-input-number>
        </el-form-item>
        <el-form-item label="出入库类型"
                      prop="InOutStockTypeName">
          <el-input v-model="viewDetailForm.InOutStockTypeName"></el-input>
        </el-form-item>
        <el-form-item label="配料任务号"
                      prop="BurdenWorkNo">
        ...
        <el-form-item label="站点ID"
                      prop="StationId">
          <el-input-number v-model="viewDetailForm.StationId"></el-input-number>
        </el-form-item>
        <el-form-item label="站点编号"
                      prop="StationCode">
        ...
        <el-form-item label="物料号"
                      prop="MaterialCode">
          <el-input v-model="viewDetailForm.MaterialCode"></el-input>
        </el-form-item>
        <el-form-item label="物料名称"
                      prop="MaterialName">
...
import { getEnumberList, getInOutStockRecordListPage, removeInOutStockRecord, editInOutStockRecord, addInOutStockRecord } from '../../api/api';
...
export default {
  ...
  data () {
    return {
      ...
      //导出组件相关 
      export_exportFileName: '出入库记录信息',//要导出的文件名 
      export_currentPageData: [],//当前页面的列表数据 
      export_columnHeader: {
        'InOutStockTypeName': '出入库类型', 'BurdenWorkNo': '配料任务号',
        'StationCode': '站点编号', 'StationName': '站点名称', 'MaterialCode': '物料号', 'MaterialName': '物料名称', 'CreateBy': '创建者', 'CreateTime': '创建时间'
      ...
      c_searchFormInputPlaceholder: '请输入出入库类型/配料任务号/站点编号/站点名称/物料号/物料名称/操作说明/创建者',//要给子搜索组件传递的值 
      c_searchFormInputAttrs: ['InOutStockTypeName', 'BurdenWorkNo', 'StationCode', 'StationName', 'MaterialCode', 'MaterialName', 'OperationRemark', 'CreateBy'],//要给子搜索组件传递的属性名 
  ...
  methods: {
    getEnumberListForInOutStockTypeEnum () {
      let para = {
        category: 'InOutStockTypeEnum'
      }
      getEnumberList(para).then((res) => {
        //给搜索框中的select控件赋值
        let propValue = "InOutStockType";
    ...
    //点击按钮【查询】 
    _getInOutStockRecords () {
      this.page = 1
      this.getInOutStockRecords()
    },
    //获取列表 
    getInOutStockRecords (formValue) {
      //this.total = 0 
      //this.InOutStockRecords = null 
      let para = this.getPostParam('1');
      this.listLoading = true;
      getInOutStockRecordListPage(para).then((res) => {
        if (res.data.success) {
          this.total = res.data.response.dataCount;
          this.InOutStockRecords = res.data.response.data;
        } else {
          this.$message({
    ...
    handleDel (selRow) {
      ...
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        this.listLoading = true;
        let para = { id: row.Id };
        removeInOutStockRecord(para).then((res) => {
      ...
      this.$refs.addForm.validate((valid) => {
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            if (this.isAdd) {//新增 
              addInOutStockRecord(para).then((res) => {
                if (util.isEmt.format(res)) {
              ...
              editInOutStockRecord(para).then((res) => {
                if (util.isEmt.format(res)) {
                  this.addLoading = false;
                  return;
                }
    ...
    //导出 
    handleExport () {
      this.export_currentPageData = this.InOutStockRecords
  ...
  mounted () {
    ...
    this.getEnumberListForInOutStockTypeEnum()
    this.setInitAddForm()
    this._getInOutStockRecords();
