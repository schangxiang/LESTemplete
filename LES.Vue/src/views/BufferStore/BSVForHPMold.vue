      ...
      <el-card class="box-card"
        ...
        <div v-for="o in StockInfos"
          ...
          <div class="el-col-xs-12 "
               :style="{width:singleDivWidthForBig}">
  ...
  margin-right: 3px;margin-bottom:10px;">
    ...
    <div class="el-row"
      ...
      <el-card class="box-card"
        ...
        <div v-for="o in StockInfosForTPQ"
             :key="o"
             class="text item">
          <div class="el-col-xs-12 "
    ...
    <el-dialog title="站点操作"
      ...
      <div slot="footer"
        ...
        <el-button type="danger"
                   class="footSty"
                   icon="fa fa-cubes"
                   @click="doClick(6)">增加库存</el-button>
        ...
        <el-button type="danger"
                   v-if="isShowRemoveAll"
                   class="footSty"
                   icon="el-icon-delete"
                   @click="doClick(7)">一键清空高压内模托盘区</el-button>
      </div>
    </el-dialog>
  </section>
</template> 
...
import { getStockForKCQY_GYNMTPQStockVisual, getStockForKCQY_GYNMZBQStockVisual, addStoreByStation, updateStationIsLock, updateStationIsDeleted, getStockByArea, deleteStockByStationId, deleteAllAllEmptySalverStation, getCodeItemsBySetCodeForGX, getStockInfoCollectListPage } from '../../api/api';
...
export default {
  ...
  data () {
    return {
      isShowRemoveAll: false,
      mainFormVisible: false,
      mainForm: {
        OperationRemark: ''
      },
      singleDivWidthForBigForTPQ: "33%",
      singleDivWidthForBig: "14%",
      singleDivWidthForZPQ: '100%',
      singleDivWidth: '10%',
      singleDivWidthForTPQ: '30%',
      StockInfos: [],//高压内模准备区
      StockInfosForTPQ: [],//高压内模托盘区
      listLoading: false,
      //定时相关
      timingValue: 10, // 定时频率
  ...
  methods: {
    ...
    handleAllEmptySalverStation () {
      this.$confirm('确认要删除高压内模托盘区所有的空托盘信息吗?', '提示', {
        type: 'warning'
      }).then(() => {
        this.listLoading = true;
        let para = {
          storeAreaEnumValue: 'KCQY_GYNMTPQ',//高压内模托盘区
          remark: '清空 高压内模托盘区所有的空托盘'
        };
    ...
    handleAddStoreByStation () {
      this.$confirm("您确定要将站点[" + this.mainForm.StationName + "] 增加物料 吗", '提示', {
        type: 'warning'
      }).then(() => {
        this.listLoading = true;
        let _CodeItemsByCode = '';
        if (this.mainForm.StoreAreaCode == "KCQY_GYNMTPQ") {
          _CodeItemsByCode = 'GGWL_KTP' //空托盘的物料
        } else if (this.mainForm.StoreAreaCode == "KCQY_GYNMZBQ") {
          _CodeItemsByCode = 'GGWL_GYNM' //高压内膜的物料
        }
        let para = {
    ...
    getStockInfos (formValue) {
      ...
      // 获取高压内模托盘区
      getStockForKCQY_GYNMTPQStockVisual(param).then((res) => {
        if (res.data.success) {
          this.StockInfosForTPQ = res.data.response;
        } else {
          this.$message({
            message: res.data.msg,
            type: 'error'
      ...
      getStockForKCQY_GYNMZBQStockVisual(param).then((res) => {
        if (res.data.success) {
  ...
  mounted () {
    if (isMobile()) {
      ...
      this.singleDivWidthForTPQ = "30%"
      this.singleDivWidthForBig = "100%"
      this.singleDivWidthForBigForTPQ = "100%"
    } else {
      this.drawerSize = "550px"//drawner宽度设置 
      this.singleDivWidth = "14%"
      this.singleDivWidthForTPQ = "30%"
      this.singleDivWidthForBig = "14%"
      this.singleDivWidthForBigForTPQ = "33%"
    }
