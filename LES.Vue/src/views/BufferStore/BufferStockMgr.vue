<template>
  <section>
    ...
    <!--清空库存界面-->
    <el-dialog title="清空库存"
               :visible.sync="clearStockForSingleStationFormVisible"
               :close-on-click-modal="false">
      <el-form :model="clearStockForSingleStationForm"
               size="small"
               label-width="80px"
               ref="clearStockForSingleStationForm">
        <el-form-item label="站点编号"
                      prop="StationCode">
          <el-input v-model="clearStockForSingleStationForm.StationCode"
                    disabled>
          </el-input>
        </el-form-item>
        <el-form-item label="站点名称"
                      prop="StationName">
          <el-input v-model="clearStockForSingleStationForm.StationName"
                    disabled></el-input>
        </el-form-item>
        <el-form-item label="操作说明"
                      prop="manualHandlerRemark"
                      :rules="[{ required: true, message: '操作说明不能为空'}]">
          <el-input v-model="clearStockForSingleStationForm.manualHandlerRemark"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer"
           class="dialog-footer">
        <el-button @click.native="clearStockForSingleStationFormVisible = false"
                   icon="fa fa-power-off">取消</el-button>
        <el-button type="primary"
                   @click.native="do_handleDeleteStockByStationId"
                   icon="fa fa-send"
...
import { deleteStockByStationId, deleteAllAllEmptySalverStation, getCodeItemsBySetCodeForGX, getStockInfoCollectListPage, removeStockInfo, editStockInfo, addStockInfo } from '../../api/api';
...
export default {
  ...
  data () {
    return {
      ...
      //清空站点库存表单
      clearStockForSingleStationForm: {
        manualHandlerRemark: ''
      },
      clearStockForSingleStationFormVisible: false,
  ...
  methods: {
    ...
    getPostParam (flag) {
      ...
      para.RequestFlag = 'BufferStockMgr'
      para.page = this.page
    ...
    //显示 清除某个站点的库存
    handleDeleteStockByStationId (selRow) {
      let row = {};
      ...
      //绑定数据
      this.clearStockForSingleStationForm = {
        manualHandlerRemark: ''
      }
      this.clearStockForSingleStationForm.StationCode = row.StationCode
      this.clearStockForSingleStationForm.StationName = row.StationName
      this.clearStockForSingleStationForm.StationId = row.StationId
      this.clearStockForSingleStationForm.manualHandlerRemark = ''
      this.clearStockForSingleStationFormVisible = true
    },
    //根据站点ID删除库存 
    do_handleDeleteStockByStationId () {
      ...
      this.$refs.clearStockForSingleStationForm.validate((valid) => {
        if (valid) {
          this.$confirm('确认要清空站点 [' + row.StationName + '] 的库存吗?', '提示', {
            type: 'warning'
          }).then(() => {
            this.listLoading = true;
            debugger
            let para = {
              stationId: this.clearStockForSingleStationForm.StationId,
              manualHandlerRemark: this.clearStockForSingleStationForm.manualHandlerRemark,
            };
            deleteStockByStationId(para).then((res) => {
              ...
              if (res.data.success) {
                ...
                this.clearStockForSingleStationFormVisible = false
              } else {
                this.$message({
                  message: res.data.msg,
                  type: 'error'
