...
import { addStoreByStation, updateStationIsLock, updateStationIsDeleted, getStockForLVDeMoldPalletVisual, getStockForLVDeMoldVisual, deleteStockByStationId, clearStockByArea, getCodeItemsBySetCodeForGX, getStockInfoCollectListPage } from '../../api/api';
...
export default {
  ...
  methods: {
    ...
    handleAllEmptySalverStation () {
      this.$confirm('确认要删除脱模内外模外协存放区所有的物料信息吗?', '提示', {
        type: 'warning'
      }).then(() => {
        this.listLoading = true;
        let para = {
          storeAreaEnumValue: 'KCQY_TMNWMWXCFQ',//脱模内外模外协存放区
          remark: '清空 脱模内外模外协存放区所有的物料信息'
        };
    ...
    classForBox (item) {
      if (item.Station_IsDeleted) {
        ...
        if (item.PlaceTypeName.indexOf('托盘') > -1) {
    ...
    singleDivClick (row) {
      ...
      if (row.StoreAreaCode === 'KCQY_TMNWMWXCFQ') {//脱模内外模外协存放区
    ...
    getStockInfos (formValue) {
      this.listLoading = true;
      getStockForLVDeMoldVisual({}).then((res) => {
        if (res.data.success) {
      ...
      //获取托盘区
      getStockForLVDeMoldPalletVisual({}).then((res) => {
        if (res.data.success) {
