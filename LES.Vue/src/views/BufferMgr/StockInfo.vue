    ...
    <el-dialog :title="dialogTitle"
      ...
      <el-form :model="addForm"
        ...
        <el-form-item label="有效期"
                      prop="ValidityDate ">
          <el-input v-model="addForm.ValidityDate"></el-input>
        </el-form-item>
...
import { getStockInfoListPage, removeStockInfo, editStockInfo, addStockInfo } from '../../api/api';
...
export default {
  ...
  data () {
    return {
      ...
      tHeader: ['SKU', '货物编号', '货物名称', '货物数量', '有效期', '入库时间', '操作说明', '创建者ID', '创建者', '修改ID', '修改者', '修改时间'],//当前页面列表的表头汉字数组，导出用 
      filterVal: ['SKU', 'CargoCode', 'CargoName', 'CargoNum', 'ValidityDate', 'InStockDate', 'OperationRemark', 'CreateId', 'CreateBy', 'ModifyId', 'ModifyBy', 'ModifyTime'],//当前页面列表的表头属性数组，导出用 
      ...
      drawerSize: "1050px",//drawner宽度设置 
      searchFormInputPlaceholder: '请输入SKU/货物编号/货物名称',//要给子搜索组件传递的值 
      searchFormInputAttrs: ['SKU', 'CargoCode', 'CargoName'],//要给子搜索组件传递的属性名 
      formOptions: [
        {
          label: 'SKU',
          prop: 'SKU',
  ...
  methods: {
    ...
    getStockInfos (formValue) {
      ...
      getStockInfoListPage(para).then((res) => {
        if (res.data.success) {
