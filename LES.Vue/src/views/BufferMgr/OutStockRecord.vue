    ...
    <el-col :span="24"
      ...
      <el-form :inline="true"
        ...
        <SearchForm :formOptions="formOptions"
                    ...
                    ref="ChildSearchForm"
                    @onSearch="getOutStockRecords" />
      </el-form>
    </el-col>

    <!--列表-->
    <el-table :data="OutStockRecords"
              :height="tableHeight"
    ...
    <el-dialog :title="dialogTitle"
      ...
      <el-form :model="addForm"
        ...
        <el-form-item label="出库时间"
          ...
          <el-input v-model="addForm.OutStockDate"></el-input>
        </el-form-item>
...
import { getOutStockRecordListPage, removeOutStockRecord, editOutStockRecord, addOutStockRecord } from '../../api/api';
...
export default {
  ...
  data () {
    return {
      //导出组件相关 
      exportFileName: '出库管理信息',//要导出的文件名 
      currentPageData: [],//当前页面的列表数据 
      tHeader: ['叫料单号', '托盘号', '工单号', 'SKU', '货物编号', '货物名称', '货物数量', '出库时间', '创建者', '创建时间', '修改者', '修改时间'],//当前页面列表的表头汉字数组，导出用 
      filterVal: ['CallMaterialCode', 'SalverCode', 'WorkOrderCode', 'SKU', 'CargoCode', 'CargoName', 'CargoNum', 'OutStockDate', 'CreateBy', 'CreateTime', 'ModifyBy', 'ModifyTime'],//当前页面列表的表头属性数组，导出用 
  ...
  methods: {
    ...
    //获取列表 
    getOutStockRecords (formValue) {
      //this.total = 0 
      //this.OutStockRecords = null 
      let para = this.getPostParam('1');
      this.listLoading = true;
      getOutStockRecordListPage(para).then((res) => {
        if (res.data.success) {
          this.total = res.data.response.dataCount;
          this.OutStockRecords = res.data.response.data;
        } else {
          this.$message({
    ...
    handleDel (selRow) {
      ...
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        this.listLoading = true;
        let para = { id: row.Id };
        removeOutStockRecord(para).then((res) => {
      ...
      this.$refs.addForm.validate((valid) => {
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            if (this.isAdd) {//新增 
              addOutStockRecord(para).then((res) => {
              ...
              editOutStockRecord(para).then((res) => {
    ...
    //导出 
    handleExport () {
      this.currentPageData = this.OutStockRecords
