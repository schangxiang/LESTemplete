<template>
  <section>
    ...
    <el-table :data="ProcedureRoles"
      ...
      <el-table-column prop="BindStationName"
                       label="绑定站点"
                       align="center">
      </el-table-column>
      ...
      <el-table-column label="操作"
        ...
        <template scope="scope">
          <el-button size="small"
                     ...
                     @click="handleEdit(scope.row)">站点绑定</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-form :model="addForm"
        ...
        <el-form-item label="登录名"
          ...
          <el-input disabled="true"
                    v-model="addForm.uLoginName"></el-input>
        </el-form-item>
        <el-form-item label="真实名"
                      prop="uRealName">
          <el-input disabled="true"
                    v-model="addForm.uRealName"></el-input>
        </el-form-item>
        <el-form-item label="绑定站点"
                      prop="BindStationList">
          <el-cascader v-model="addForm.BindStationList"
                       style="width:100%"
                       clearable
                       :props="cascader_props"
                       :options="stationListForCascader"></el-cascader>
        </el-form-item>
    ...
    <el-dialog title="查看"
      ...
      <el-form :model="viewDetailForm"
        ...
        <el-form-item label="登录名"
                      prop="uLoginName">
          <el-input disabled="true"
                    v-model="viewDetailForm.uLoginName"></el-input>
        </el-form-item>
        <el-form-item label="真实名"
                      prop="uRealName">
          <el-input disabled="true"
                    v-model="viewDetailForm.uRealName"></el-input>
        </el-form-item>
        <el-form-item label="绑定站点"
                      prop="BindStationList">
          <el-cascader v-model="viewDetailForm.BindStationList"
                       style="width:100%"
                       clearable
                       :props="cascader_props"
                       :options="stationListForCascader"></el-cascader>
...
import { getStaionsByProcedureForCascader, getCodeItemsBySetCodeForPublic, getUserStationListPage, removeUserStation, editUserStation, addUserStation } from '../../api/api';
...
export default {
  ...
  data () {
    return {
      ...
      //导出组件相关 
      export_exportFileName: '用户站点绑定查询视图信息',//要导出的文件名 
      export_currentPageData: [],//当前页面的列表数据 
      export_columnHeader: {
        'uLoginName': '登录名',
        'uRealName': '用户真实名', 'BindStationName': '绑定站点', 'Remark': '备注',
      ...
      c_searchFormInputPlaceholder: '请输入登录名/用户真实名/绑定站点/备注',
      c_searchFormInputAttrs: ['uLoginName', 'uRealName', 'BindStationName', 'Remark'],//要给子搜索组件传递的属性名 
      ...
      stationListForCascader: []
    }
  },
  methods: {
    //获取站点，按照 工序的 分级结构 
    GetStaionsByProcedureForCascader () {
      getStaionsByProcedureForCascader().then((res) => {
        if (res.data.success) {
          this.stationListForCascader = res.data.response
        } else {
          this.$message({
    ...
    getList (formValue) {
      ...
      getUserStationListPage(para).then((res) => {
        if (res.data.success) {
  ...
  mounted () {
    this.GetStaionsByProcedureForCascader()
    this._getList();
